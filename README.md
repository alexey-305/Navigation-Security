...
- **MVVM** — для основных экранов (`FeedViewController`/`FeedViewModel`,
  `LoginViewController`/`LoginViewModel`); остальные экраны (агрегатор цитат,
  карта, избранное) пока написаны как MVC — это известное ограничение,
  see «Что не сделано» ниже.
...
- Единая MVVM-архитектура не доведена до конца — агрегатор цитат, карта и
  избранное остаются на MVC.
